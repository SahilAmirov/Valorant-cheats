...
serialcomm = serial.Serial("COM6",115200,timeout = 0)
...
silent_toggle = [True]
no_fov_cooldown = [True]                                                                        #best_half.onnx | best_half.pt
model = torch.hub.load(r'C:\Users\SahilAmirov\Desktop\Desk\cheats\yolov5' , 'custom', path= r'C:\Users\SahilAmirov\Desktop\Desk\cheats\best_half.onnx',source='local')
model.conf = 0.40
model.maxdet = 10
model.apm = True 
model.classes = [1]
...
while True:
    ...
    df = model(screenshot, size=[736,736]).pandas().xyxy[0]
    ...
    if closest_part != -1:
        ...
        if aim_assist == True and closest_part_distance < activation_range and send_next[0] == True:
            ...
            serialcomm.write(data.encode())
            send_next[0] = False
            thread = threading.Thread(target=cooldown, args=(send_next,0.035,))
            thread.start()
